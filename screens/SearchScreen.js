...
export default function SearchScreen() {
  ...
  return (
    <SafeAreaView className="bg-neutral-50 flex-1 text-black pt-16">



      <View className="mx-4 mb-3 flex-row justify-between items-center border border-neutral-200 rounded-full bg-input">
        <TextInput
          ref={inputRef}
              ...
              <TouchableWithoutFeedback key={index} onPress={() => handleClick(item)}>
                <View className="w-1/2 mb-4 px-2">
                  <View className="rounded-lg overflow-hidden border border-neutral-300">
                    <Image
                      source={item.volumeInfo.imageLinks?.thumbnail ? { uri: item.volumeInfo.imageLinks.thumbnail } : require('../assets/no-cover.jpg')}
